...
static int is_command_position(const char *buffer, size_t cursor_pos) {
    ...
    if (i >= cursor_pos) return 1; /* Cursor is in leading whitespace */
    while (i < cursor_pos && !isspace(buffer[i])) {
        i++;
    }
    while (i < cursor_pos && isspace(buffer[i])) {
...
static char *find_common_prefix(char **candidates, size_t count, const char *word) {
    if (count == 0) return NULL;
    if (count == 1) return strdup(candidates[0]);
...
static char **get_default_completions(const char *word, size_t *count, int is_command, char **path_prefix) {
    ...
    *count = 0;
    ...
    if (is_command) {
        ...
        char *seen = calloc(MAX_COMPLETIONS, MAX_PATH_LEN); /* Track seen names */
        size_t seen_count = 0;
...
static void apply_completion(char *buffer, size_t *len, size_t *cursor_pos,
                            size_t word_start, size_t word_end,
                            const char *completion, const char *word, int is_command,
    ...
    snprintf(full_completion, sizeof(full_completion), "%s%s%s%s",
             path_prefix, completion,
             is_directory ? "/" : "",
             add_space ? " " : "");
    ...
    if (new_len >= BUFFER_SIZE - 1) return; /* Buffer overflow check */
...
static void list_completions(CompletionState *state, int prompt_row, int prompt_col,
                            struct termios *orig_termios) {
    if (state->count == 0) return;

    if (state->count > 20) {
        char prompt[256];
        snprintf(prompt, sizeof(prompt), "\nThere are more than %zu possibilities, do you want to list them? [y/n] ", state->count);
        printf("%s", prompt);
        fflush(stdout);
