...
int utf8_validate(const char *str) {
    ...
    while (*s) {
        ...
        if (len == 0 || (len == 1 && (*s & 0x80))) return 0; // Invalid leading byte
        for (int i = 1; i < len; ++i) {
            if ((s[i] & 0xC0) != 0x80) return 0; // Check continuation bytes
        }
        s += len;
    }
    return 1;
...
const char *utf8_prev(const char *start, const char *current) {
    ...
    while (s >= start) {
        if ((*s & 0xC0) != 0x80) return s; // Find the start of the UTF-8 character
        --s;
    }
...
int utf8_char_length(const char *str) {
    ...
    int retVal = 0;
    if (c < 0x80) retVal = 1;
    if ((c & 0xE0) == 0xC0) retVal = 2;
    if ((c & 0xF0) == 0xE0) retVal = 3;
    if ((c & 0xF8) == 0xF0) retVal = 4;
    return retVal;
}
...
int utf8_char_width(const char *str) {
    ...
    if (codepoint >= 0x1100 && codepoint <= 0x115F) return 2; // Example: Wide characters
    if (codepoint >= 0x1F300 && codepoint <= 0x1F64F) return 2; // Emojis
    return 1;
}
